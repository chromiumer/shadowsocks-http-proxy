...
import (
	...
	ss "github.com/chromiumer/shadowsocks-go/shadowsocks"
...
var http_proxy *goproxy.ProxyHttpServer
...
func run(listenAddr string, is_http_proxy bool) {
	if !is_http_proxy {
		ln, err := net.Listen("tcp", listenAddr)
		if err != nil {
			log.Fatal(err)
		}
		log.Printf("starting local socks5 proxy server at %v ...\n", listenAddr)
		for {
			conn, err := ln.Accept()
			if err != nil {
		...
		http.ListenAndServe(listenAddr, http_proxy)
	}
}
...
func main() {
	...
	flag.StringVar(&proxy_type, "proxy_type", "socks5", "set local proxy type: socks5/http")
	...
	run(cmdLocal+":"+strconv.Itoa(config.LocalPort), proxy_type == "http")
}
